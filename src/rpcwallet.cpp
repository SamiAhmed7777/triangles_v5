...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    return CTrianglesAddress(keyID).ToString();
}


CTrianglesAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CTrianglesAddress(account.vchPubKey.GetID());
}
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    CTrianglesAddress address(params[0].get_str());
    if (!address.IsValid())
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
        "sendtoaddress <Trianglesaddress> <amount> [comment] [comment-to]\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                ...
                if (pwalletMain->mapAddressBook.find(CTrianglesAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CTrianglesAddress(address).Get())->second);
            }
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    // Triangles address
    CTrianglesAddress address = CTrianglesAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Triangles address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
        "sendfrom <fromaccount> <toTrianglesaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());

    string strAccount = AccountFromValue(params[0]);
    CTrianglesAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Triangles address");
    int64_t nAmount = AmountFromValue(params[2]);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: Triangles address and we have full public key:
        CTrianglesAddress address(ks);
        if (address.IsValid())
        {
            CKeyID keyID;
    ...
    return CTrianglesAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CTrianglesAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CTrianglesAddress, tallyitem>::iterator it = mapTally.find(address);
